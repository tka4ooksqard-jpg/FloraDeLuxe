...
src/
  ...
  lib/
    site-config.ts     назва, URL, SEO-дефолти, локаль
    contact-config.ts  адреса, Telegram, телефон, комерційні умови
    content/           увесь текстовий контент, типізований
    validation/        Zod-схема форми (для майбутнього підключення)
    actions/           Server Action форми (для майбутнього підключення)
...
На публічних сторінках форма заявки **тимчасово замінена** блоком
Telegram CTA (`TelegramOrderCta`). Код `LeadForm` + `submitLead` + Zod
залишено в репозиторії для підключення після затвердження сайту та
реальної доставки заявок. Персональні дані через сайт зараз не збираються.
...
2. **Замінити згенеровані / непідтверджені фото та відео** на матеріали
   компанії. Див. також `public/images/README.md`.
3. **Додати підтверджені відгуки** в `src/lib/content/reviews.ts` (поки
   масив порожній — секція на сайті не показується).
4. **Підключити доставку заявок** у `deliverLead()` і знову змонтувати
   `LeadForm` на головній / контактах замість тимчасового Telegram CTA.
5. **Задати домен:** змінна оточення `NEXT_PUBLIC_SITE_URL` (canonical,
